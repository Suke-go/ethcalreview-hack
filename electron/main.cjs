...
function createWindow() {
    ...
    if (isDev) {
        ...
        // パッケージ後は dist/ が electron/ と同じ階層にある
...
function startBackend() {
    ...
    if (isDev) {
        ...
        // 本番モード: PyInstallerでビルドしたexe（extraResourcesから）
        const exePath = path.join(process.resourcesPath, 'backend-dist', 'backend.exe');
