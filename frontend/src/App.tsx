...
import type { AnalysisResult, FormData, SessionDetail } from './types';
...
function App() {
  ...
  const [_selectedSession, setSelectedSession] = useState<SessionDetail | null>(null);
  ...
  const handleAnalyze = useCallback(async (plan: string) => {
    ...
    await analyzeSSE.start(`http://localhost:8000/api/analyze/stream`, {
      headers: {
        'X-API-Key': apiKey,
  ...
  const handleGenerate = useCallback(async () => {
    ...
    await generateSSE.start(`http://localhost:8000/api/generate/stream`, {
      headers: {
        'X-API-Key': apiKey,
        'X-LLM-Provider': provider,
        'X-LLM-Model': model,
      ...
      onResult: (result) => {
        ...
        setGeneratedSessionId(res.sessionId);
        setCurrentStep(3);
  ...
  }, [analysisResult, settings, apiKey, appConfig, generateSSE]);
                    ...
                    <div className="review-card">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setAppConfig(prev => ({
                            ...prev,
                            subInvestigators: [...prev.subInvestigators, { affiliation: '', position: '', name: '' }]
                          }));
                        }}
